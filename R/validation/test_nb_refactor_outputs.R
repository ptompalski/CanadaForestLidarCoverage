...
existing_file <- "layers/pre-processed/NB/ALS_NB.gpkg"
existing_diss_file <- "layers/pre-processed/NB/ALS_NB_diss.gpkg"

test_dir <- "scratch/nb_refactor_test"
test_file <- file.path(test_dir, "ALS_NB_refactor_test.gpkg")
test_diss_file <- file.path(test_dir, "ALS_NB_diss_refactor_test.gpkg")

dir.create(test_dir, recursive = TRUE, showWarnings = FALSE)
unlink(c(test_file, test_diss_file))

Sys.setenv(
  NB_OUTPUT_FILE = test_file,
  NB_OUTPUT_DISS_FILE = test_diss_file
)
...
results <- list(
  compare_layer(existing_file, test_file, "ALS_NB"),
  compare_layer(existing_diss_file, test_diss_file, "ALS_NB_diss")
)
...
if (!all_passed) {
  stop("NB refactor output comparison failed", call. = FALSE)
}

cat("\nNB refactor output comparison passed.\n")
