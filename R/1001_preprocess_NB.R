...
# ALS_NB %<>% st_transform(crs = the_crs)
ALS_NB %<>% st_make_valid()
# ALS_NB %<>% mutate(area = st_area(.))
...
st_write(ALS_NB, dsn = "layers/pre-processed/NB/ALS_NB.gpkg", append = F)
...
st_write(
  ALS_NB_diss,
  dsn = "layers/pre-processed/NB/ALS_NB_diss.gpkg",
  append = F
)
