...
existing_file <- "layers/pre-processed/PEI/ALS_PEI.gpkg"
existing_diss_file <- "layers/pre-processed/PEI/ALS_PEI_diss.gpkg"

test_dir <- "scratch/pei_refactor_test"
test_file <- file.path(test_dir, "ALS_PEI_refactor_test.gpkg")
test_diss_file <- file.path(test_dir, "ALS_PEI_diss_refactor_test.gpkg")

dir.create(test_dir, recursive = TRUE, showWarnings = FALSE)
unlink(c(test_file, test_diss_file))

Sys.setenv(
  PEI_OUTPUT_FILE = test_file,
  PEI_OUTPUT_DISS_FILE = test_diss_file
)
...
results <- list(
  compare_layer(existing_file, test_file, "ALS_PEI"),
  compare_layer(existing_diss_file, test_diss_file, "ALS_PEI_diss")
)
...
if (!all_passed) {
  stop("PEI refactor output comparison failed", call. = FALSE)
}

cat("\nPEI refactor output comparison passed.\n")
