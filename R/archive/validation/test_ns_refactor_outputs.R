...
existing_file <- "layers/pre-processed/NS/ALS_NS.gpkg"
existing_diss_file <- "layers/pre-processed/NS/ALS_NS_diss.gpkg"

test_dir <- "scratch/ns_refactor_test"
test_file <- file.path(test_dir, "ALS_NS_refactor_test.gpkg")
test_diss_file <- file.path(test_dir, "ALS_NS_diss_refactor_test.gpkg")

dir.create(test_dir, recursive = TRUE, showWarnings = FALSE)
unlink(c(test_file, test_diss_file))

Sys.setenv(
  NS_OUTPUT_FILE = test_file,
  NS_OUTPUT_DISS_FILE = test_diss_file
)
...
results <- list(
  compare_layer(existing_file, test_file, "ALS_NS"),
  compare_layer(existing_diss_file, test_diss_file, "ALS_NS_diss")
)
...
if (!all_passed) {
  stop("NS refactor output comparison failed", call. = FALSE)
}

cat("\nNS refactor output comparison passed.\n")
