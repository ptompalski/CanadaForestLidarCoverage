...
existing_file <- "layers/pre-processed/SK/ALS_SK.gpkg"
existing_diss_file <- "layers/pre-processed/SK/ALS_SK_diss.gpkg"

test_dir <- "scratch/sk_refactor_test"
test_file <- file.path(test_dir, "ALS_SK_refactor_test.gpkg")
test_diss_file <- file.path(test_dir, "ALS_SK_diss_refactor_test.gpkg")

dir.create(test_dir, recursive = TRUE, showWarnings = FALSE)
unlink(c(test_file, test_diss_file))

Sys.setenv(
  SK_OUTPUT_FILE = test_file,
  SK_OUTPUT_DISS_FILE = test_diss_file
)
...
results <- list(
  compare_layer(existing_file, test_file, "ALS_SK"),
  compare_layer(existing_diss_file, test_diss_file, "ALS_SK_diss")
)
...
if (!all_passed) {
  stop("SK refactor output comparison failed", call. = FALSE)
}

cat("\nSK refactor output comparison passed.\n")
