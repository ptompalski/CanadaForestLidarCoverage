...
existing_file <- "layers/pre-processed/AB/ALS_AB.gpkg"
existing_diss_file <- "layers/pre-processed/AB/ALS_AB_diss.gpkg"

test_dir <- "scratch/ab_refactor_test"
test_file <- file.path(test_dir, "ALS_AB_refactor_test.gpkg")
test_diss_file <- file.path(test_dir, "ALS_AB_diss_refactor_test.gpkg")

dir.create(test_dir, recursive = TRUE, showWarnings = FALSE)
unlink(c(test_file, test_diss_file))

Sys.setenv(
  AB_OUTPUT_FILE = test_file,
  AB_OUTPUT_DISS_FILE = test_diss_file
)
...
results <- list(
  compare_layer(existing_file, test_file, "ALS_AB"),
  compare_layer(existing_diss_file, test_diss_file, "ALS_AB_diss")
)
...
if (!all_passed) {
  stop("AB refactor output comparison failed", call. = FALSE)
}

cat("\nAB refactor output comparison passed.\n")
