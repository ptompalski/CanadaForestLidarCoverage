...
existing_file <- "layers/pre-processed/QC/ALS_QC.gpkg"
existing_diss_file <- "layers/pre-processed/QC/ALS_QC_diss.gpkg"

test_dir <- "scratch/qc_refactor_test"
test_file <- file.path(test_dir, "ALS_QC_refactor_test.gpkg")
test_diss_file <- file.path(test_dir, "ALS_QC_diss_refactor_test.gpkg")

dir.create(test_dir, recursive = TRUE, showWarnings = FALSE)
unlink(c(test_file, test_diss_file))

Sys.setenv(
  QC_OUTPUT_FILE = test_file,
  QC_OUTPUT_DISS_FILE = test_diss_file
)
...
results <- list(
  compare_layer(existing_file, test_file, "ALS_QC"),
  compare_layer(existing_diss_file, test_diss_file, "ALS_QC_diss")
)
...
if (!all_passed) {
  stop("QC refactor output comparison failed", call. = FALSE)
}

cat("\nQC refactor output comparison passed.\n")
